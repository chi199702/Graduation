...
from carProject.Detector import Detector
from carProject.OCR import *
import time



PATH = '/home/wdk/桌面/carproject/OCR'

class LPR():
    def __init__(self,folder,opt):
        """
        ...
        self.detect_object = Detector(opt=opt)
        self.decode_ocr = recognition(opt=opt)
    ...
    def to_refine(self,image, pts, scale=3.0):
        ...
        #-----------------
        #提高速度一：carPlateRecognize(OCR)/licenseplate/hyperlpr.py
        #说明：cv2.estimateAffine2D函数鲁棒性更好，抗噪声能力更强，但是计算成本要远高于cv2.estimateAffinePartial2D
        #因此，考虑到实时性，我默认使用了cv2.estimateAffinePartial2D，后期如果觉着正确性（倾斜校正为效果更好的正视图视角）更重要，可以考虑换成cv2.estimateAffine2D
        #-----------------
        #mat_,_ = cv2.estimateAffine2D(org_pts, target_pts)
        mat_,_ = cv2.estimateAffinePartial2D(org_pts, target_pts)
    ...
    def finetune(self,image_, stage=2):
        ...
        for i,img in enumerate(image_):
            tof.append(cv2.resize(img.copy(),(120,48)))
        blobs = cv2.dnn.blobFromImages(np.array(tof),size=(120, 48), swapRB=False, mean=(127.5, 127.5, 127.5), scalefactor=0.0078125, crop=False)
        self.refine_net.setInput(blobs)
        h, w, c = image_[0].shape
        pts = (self.refine_net.forward("conv6-3").reshape(len(image_),4, 2) * np.array([w, h])).astype(np.int)

        g = []
        for i,single_pts in enumerate(pts):
            g.append(self.to_refine(image_[i],single_pts))
        blobs = cv2.dnn.blobFromImages(np.array(g), size=(120, 48), swapRB=False, mean=(127.5, 127.5, 127.5), scalefactor=0.0078125, crop=False)
        self.refine_net.setInput(blobs)
        h, w, c = g[0].shape
        pts = (self.refine_net.forward("conv6-3").reshape(len(image_),4, 2) * np.array([w, h])).astype(np.int)
        cropped = []
        for i,single_pts in enumerate(pts):
            cropped.append(self.affine_crop(g[i],single_pts))
        return cropped
        ...
        # h, w, c = image_.shape
        # # print(self.refine_net.forward("prelu5") )
    ...
    def segmentation_free_recognition(self,src):
        ...
        test = self.modelRecognition.forward()
    ...
    def plate_recognition(self,image):
        ...
        """
        detectResults = self.detect_object.detect_yolo(image)
        ...
        if detectResults == None:
            ...
            for i,data in enumerate(detectResults):
                for j,(conf,crop_axis,croped_img) in enumerate(zip(*data)):
                    # print(len(crop_axis))
                    iret_conf.append(conf)
                    iret_box.append(crop_axis)
                    cropped = self.loose_crop(image[i], crop_axis, 120 / 48)
                    cropped_image_list.append(cropped)
                list_number.append(len(data[2]))
            cropped_finetuned_list = self.finetune(cropped_image_list)
            iret_ocr = self.decode_ocr.decode_char(cropped_finetuned_list)
            # print("list_number :")
            # print(list_number)
            ...
            return iret_conf,iret_box,iret_ocr
