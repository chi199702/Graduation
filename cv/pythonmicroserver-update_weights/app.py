...
def return_results():
    ...
    try:
        ...
        for index in range(length):
            file = request.files["file{}".format(index)].read()
            data = np.frombuffer(file,dtype=np.uint8)
            img = cv2.imdecode(np.array(data),cv2.IMREAD_COLOR)
            img_list.append(img)
...
#    with mutex:
    ...
    if infor:
        ...
        for i,(iret_conf,iret_box,iret_ocrin) in enumerate(zip(*infor)):
            # print(iret_box)
            count += 1
            cv2.rectangle(img,(int(iret_box[0]),int(iret_box[1])),(int(iret_box[2]),int(iret_box[3])),(0, 255, 0), 2)
